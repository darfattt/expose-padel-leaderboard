...
- `lib/rating.ts` — `computeRating()` blends win-rate / point-diff / ppg z-scores (weights in `RATING_WEIGHTS`) into a score on Playtomic's 0.0–7.0 level scale (capped at `MAX_RATING` = 7; mid-field ≈ 3.5). **Reliability gates** (`RELIABILITY_TIERS` + `capForReliability`) hold a rating just below each upper band until the player clears that band's games+wins bar — one gate per level (4: 8g/5w, 5: 14g/9w, 6: 20g/13w, 7: 30g/20w). This is our concrete take on Playtomic's "reliability" (high levels need a proven, ~60–67%-winning sample, not a fixed match count), so a thin hot streak can't fake an Elite rating. Players under `MIN_GAMES_RANKED` (3) are **provisional** (unranked).
