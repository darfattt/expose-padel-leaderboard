...
- `lib/scoring.ts` — events come in different point scales (a "first to 21" game vs a fixed-sum "to 5" game like 3-2 / 5-0). `events.points_per_game` records each event's basis (auto-detected at upload by `detectPointsPerGame`, confirmable in the form). The SQL views and `aggregateResults` expose `norm_*` aggregates with every game scaled by `21 / points_per_game` (`normFactor`); the rating layer reads those so the reliability gates (absolute net points) and the close-game threshold mean the same thing across scales, while raw `points_for`/`point_diff` stay verbatim for display. For 21-point data the factor is 1 (a no-op). The skill z-scores are scale-invariant within a field; normalization is what keeps mixed-scale fields and the absolute gates honest.
- `player_career_stats` (SQL view) → `CareerStatRow` raw aggregates per player (plus `norm_*` parallels).
