...
- `lib/rating.ts` — `computeRating()` blends win-rate / point-diff / ppg z-scores (weights in `RATING_WEIGHTS`) into a 0.0–10.0 Playtomic-style score. Players under `MIN_GAMES_RANKED` (3) are **provisional** (unranked).
- `lib/levels.ts` — maps a 0–10 rating to a Playtomic level band (category, emoji badge, accent color).
